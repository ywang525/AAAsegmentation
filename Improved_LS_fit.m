function [y1,x1,idx,varargout] = Improved_LS_fit(phi)
...
[y1 x1] = ind2sub(size(phi),idx);

x=x1(:); % convert data to column vectors
y=y1(:);
if numel(x)~=numel(y) || numel(x)<5
    idx = find( phi <= 1.8 & phi >= -1.8 );  % get the curve's narrow band
    [y1 x1] = ind2sub(size(phi),idx);
    x=x1(:); % convert data to column vectors
    y=y1(:);
    if numel(x)~=numel(y) || numel(x)<5
    idx = find( phi <= 2.8 & phi >= -2.8 );  % get the curve's narrow band
    [y1 x1] = ind2sub(size(phi),idx);
    x=x1(:); % convert data to column vectors
    y=y1(:);
    end
